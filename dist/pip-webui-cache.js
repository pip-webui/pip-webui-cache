        ...
        var getDefaultParams = function (params) {
            ...
            if (params) {
                if (params.hasOwnProperty('offset')) {
                    ret.offset = parseInt(params.offset, 10);
                }
                if (params.hasOwnProperty('limit')) {
                    ret.limit = parseInt(params.limit, 10);
                }
            }
            return ret;
        };
                        ...
                        if (match) {
                            switch (config.method) {
                                case 'GET':
                                    switch (ik) {
                                        case 'item':
                                            ...
                                            return { value: pipCache.getItem(model.name, interceptor.getKey(groups), interceptor.options)
                                                    .then(function (item) {
                                                    if (!item) {
                                        ...
                                        case 'collection':
                                            var params_1 = interceptor.getParams ? interceptor.getParams(config.params) : getDefaultParams(config.params);
                                            return { value: pipCache.getItems(model.name, params_1, interceptor.options)
                                                    .then(function (items) {
                                                    if (!items) {
                                                        config.onResponse = function (body) {
                                                            var its = interceptor.responseModify
                                                                ? interceptor.responseModify.responseToItems(body) : body;
                                                            pipCache.setItems(model.name, its, { params: params_1, options: interceptor.options });
                                                        };
                                                        return config;
                                                    }
                                                    else {
                                ...
                                case 'DELETE':
                                    switch (ik) {
                                        case 'item':
                                            var groups_1 = match.groups;
                                            config.onResponse = function (body) {
                                                pipCache.deleteItems(model.name, [interceptor.getKey(groups_1)]);
                                            };
                                            break;
                                        default:
                                            break;
...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var CacheCollectionParams = (function () {
    function CacheCollectionParams() {
    }
    return CacheCollectionParams;
}());
exports.CacheCollectionParams = CacheCollectionParams;
var CacheInterceptorOptions = (function () {
    function CacheInterceptorOptions() {
    }
    return CacheInterceptorOptions;
}());
exports.CacheInterceptorOptions = CacheInterceptorOptions;
...
var CacheService = (function () {
    ...
    CacheService.prototype.getItems = function (modelName, params, options) {
        return __awaiter(this, void 0, void 0, function () {
            var model, db, modelKey, totalDetails, maxAge, total, offset, limit, indexes, ids, _a, items, reads, indexes, ids, _b, items, reads, _c, items, reads;
            return __generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, db.table('meta').get('total')];
                    case 1:
                        totalDetails = _d.sent();
                        maxAge = options && options.maxAge || model.options.maxAge;
                        total = totalDetails && (totalDetails.lastRead + maxAge >= new Date().valueOf())
                            ? totalDetails.total : undefined;
                        if (this.config.enableLogs) {
                            console.groupCollapsed('[PipCache] GET collection of items');
                            console.log('Params: ', params);
                        }
                        if (!(params && Object.keys(params).length)) return [3 /*break*/, 10];
                        offset = params.hasOwnProperty('offset') ? params.offset : 0;
                        limit = params.hasOwnProperty('limit') ? params.limit : undefined;
                        if (!(limit !== undefined)) return [3 /*break*/, 6];
                        return [4 /*yield*/, db.table('indexes').where('idx').between(offset, offset + limit, true, false).toArray()];
                    case 2:
                        indexes = _d.sent();
                        if (!(indexes.length !== limit && (total === undefined || indexes.length !== total - offset))) return [3 /*break*/, 3];
                        if (this.config.enableLogs) {
                    ...
                    case 6:
                        if (!(total !== undefined)) return [3 /*break*/, 9];
                        return [4 /*yield*/, db.table('indexes').where('idx').aboveOrEqual(offset).toArray()];
                    case 7:
                        indexes = _d.sent();
                    ...
                    case 8:
                        _b = _d.sent(), items = _b[0], reads = _b[1];
                        if (!items || items.length !== indexes.length || !reads || reads.length !== indexes.length) {
                            if (this.config.enableLogs) {
                                console.log('Not all items presented in cache');
                                console.groupEnd();
                    ...
                    case 10:
                        if (!(total !== undefined)) return [3 /*break*/, 12];
                        return [4 /*yield*/, Promise.all([
                                db.table('items').toArray(),
                                db.table('lastRead').toArray()
                            ])];
                    case 11:
                        _c = _d.sent(), items = _c[0], reads = _c[1];
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 1:
                        _a = _b.sent(), expire = _a[0], it = _a[1];
                        if (!options) return [3 /*break*/, 3];
                        if (!options.removeTotal) return [3 /*break*/, 3];
                        return [4 /*yield*/, db.table('meta').delete('total')];
                    case 2:
                        _b.sent();
                        _b.label = 3;
                    case 3:
    ...
    CacheService.prototype.setItems = function (modelName, items, payload) {
        return __awaiter(this, void 0, void 0, function () {
            var _this = this;
            var model, modelKey, db, lr, offset, ids, indexes, promises, total;
            return __generator(this, function (_a) {
                model = this.getModel(modelName);
                modelKey = model.options.key || 'id';
                db = this.getDb(model);
                lr = new Date().valueOf();
                offset = payload && payload.params && payload.params.offset || 0;
                ids = items.map(function (it) { return it[modelKey]; });
                indexes = ids.map(function (id, idx) { return ({ id: id, idx: idx + offset }); });
                promises = [
                ...
                if ((payload && payload.params && payload.params.limit && items && items.length < payload.params.limit)
                    ...
                    promises.push(db.table('meta').put({ lastRead: new Date().valueOf(), total: total }, 'total'));
                }
            ...
            return __generator(this, function (_a) {
                return [2 /*return*/, new Promise(function (resolve) {
                        ...
                        for (var _i = 0, dbs_1 = dbs; _i < dbs_1.length; _i++) {
                            var name_1 = dbs_1[_i];
                            try {
                                var db = _this.openedDbs.has(name_1) ? _this.openedDbs.get(name_1) : new Dexie.Dexie(name_1);
...
},{"./cache-config.service":2,"./cache.interceptor":3,"./cache.models":4,"./cache.service":5}],7:[function(require,module,exports){
"use strict";
function __export(m) {
    for (var p in m) if (!exports.hasOwnProperty(p)) exports[p] = m[p];
}
